...
def predict_save_to_queue(preprocess_fn, q, list_of_lists, output_files, segs_from_prev_stage, classes, worker_id):
    errors_in = []
    for i, l in enumerate(list_of_lists):
        try:
            ...
            if segs_from_prev_stage[i] is not None:
                assert isfile(segs_from_prev_stage[i]) and segs_from_prev_stage[i].endswith(".nii"), "segs_from_prev_stage" \
                                                                                                  " must point to a " \
                                                                                                  "segmentation file"
    ...
    if len(errors_in) > 0:
        ...
        print(f"This worker[{worker_id}] has ended successfully, no errors to report")
...
def predict_from_folder(model, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
    ...
    output_files = [join(output_folder, i + ".nii") for i in case_ids]
    all_files = subfiles(input_folder, suffix=".nii", join=False, sort=True)
